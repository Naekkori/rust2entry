...
| 11 | `lib::compile` — 전체 조립 (object 매칭, thread 분리, functions/messages emit, Entry 형식) | ✅ | 114/114 |
...
- [ ] **다음 우선순위 (블록 추가)**
  ...
  - [x] 형태: `show` / `hide` (오브젝트 보이기/숨기기), `dialog` (말하기)
...
**빌드/테스트 명령**:
```
cargo test                  # 전체 (entryc 5 + codegen 9 + compile 114 + parse 26 = 154 통과)
...
**다음 할 일 추천 순서**:
1. 형태 — `dialog_time` (시간 말하기) — dialog 변형
2. 형태 — `change_to_some_shape` / `change_to_next_shape` (모양 변경)
3. 변수 — 리스트 (값 추가/삭제/길이) — list kind 활용
