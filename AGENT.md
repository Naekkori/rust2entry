...
| 11 | `lib::compile` — 전체 조립 (object 매칭, thread 분리, functions/messages emit, Entry 형식) | ✅ | 137/137 |
...
**빌드/테스트 명령**:
```
cargo test                  # 전체 (entryc 5 + codegen 9 + compile 137 + parse 26 = 177 통과)
...
**다음 할 일 추천 순서**:
1. 형태 — `set_scale_size` / `reset_scale_size` (크기 설정/리셋)
