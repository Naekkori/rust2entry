...
| 11 | `lib::compile` — 전체 조립 (object 매칭, thread 분리, functions/messages emit, Entry 형식) | ✅ | 129/129 |
...
**빌드/테스트 명령**:
```
cargo test                  # 전체 (entryc 5 + codegen 9 + compile 129 + parse 26 = 169 통과)
...
**다음 할 일 추천 순서**:
1. 형태 — `change_effect_amount` (효과 정하기) — add 와 같은 패턴
