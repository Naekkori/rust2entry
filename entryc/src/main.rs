...
fn inject_memo_blocks(
    ...
    let build_sec = std::time::SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    // sources 의 (name, _) 와 rs_paths 의 stem 을 매핑해 정확한 경로 사용.
    ...
    for o in objects.iter_mut() {
        ...
        let script_raw = o.get(script_key).cloned().unwrap_or(Value::Null);
        let mut threads: Vec<Vec<Value>> = if script_raw.is_null() {
            Vec::new()
        } else if let Value::Array(arr) = &script_raw {
            arr.iter()
                .map(|t| match t {
        ...
        let encoded = serde_json::to_string(&Value::Array(
            threads.into_iter().map(Value::Array).collect(),
        ))
        .map_err(|e| format!("script json encode: {e}"))?;
        o[script_key] = Value::String(encoded);
    }
    Ok(())
}

/// Entry 블록 JSON id — 시스템 clock 기반 짧은 hex. 충돌 가능성 무시 (16 hex).
...
fn run_build(
    ...
    // EntryJS 가 코드 영역에 노란 박스로 표시한다. 변환 후 어떤 .rs 가 어떤 object
    // 로 emit 됐는지, 빌드 시각, 변수 개수 등을 워크스페이스 안에서 즉시 확인 가능.
    if let Err(e) = inject_memo_blocks(&mut final_project, &sources_ref, rs_files) {
        eprintln!("memo inject warning: {e}");
    }
