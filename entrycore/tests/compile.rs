...
/// 같은 source 에서 같은 변수명을 두 번 선언하면 컴파일 실패.
#[test]
fn compile_duplicate_variables_in_same_source_fail() {
    let src = r#"
        fn when_start() {
            let x = 1;
            let x = 2;
    ...
    "#;
    let r = compile(&[("obj", src)], &empty_project());
    assert!(r.is_err(), "같은 scope 에서 중복 변수 선언 시 컴파일 실패해야 함");
    let err = r.unwrap_err();
    let msg = err.to_string();
    assert!(
        msg.contains("x") && msg.contains("multiple"),
        "에러 메시지에 변수명 포함: {msg}"
    );
}

/// 같은 변수명이 여러 source 에 흩어져 있으면 둘 다 에러 (cross-source 도 unique).
#[test]
fn compile_duplicate_variables_across_sources_fail() {
    let a = "fn when_start() { let x = 1; }";
    let b = "fn when_start() { let x = 2; }";
    let r = compile(&[("a", a), ("b", b)], &empty_project());
    assert!(r.is_err(), "cross-source 중복 변수명도 컴파일 실패");
}
...
fn compile_ask_and_wait_roundtrip() {
    ...
    let p1 = entrycore::parse::parse(src).expect("parse1");
    let v = compile(&[("obj", src)], &empty_project()).expect("compile").0;
    let vars = entrycore::codegen::collect_var_map(&p1).unwrap();
    let objects = v["objects"].as_array().unwrap();
    let obj_script_str = objects[0]["script"].as_str().expect("script str");
    let p2 = program_from_script_string_with_vars(obj_script_str, &vars).expect("deparse");
    match &p2.stmts[0] {
