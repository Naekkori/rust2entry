...
pub fn from_stmt(stmt: &crate::ir::Stmt) -> crate::Result<Block> {
    match stmt {
        ...
        Stmt::For { var:_, iter: _, body:_ }=>{
            Err(UnmappedBlock("for-range not support".into()))
        }
...
pub fn to_value(block: &Block) -> crate::Result<Value> {
    ...
    let params = match block {
        Block::SetVar { variable, value } => vec![
            variable_param(variable),
            param_to_value(value),
            Value::Null,
        ],
        Block::ChangeVar { variable, value } => vec![
            variable_param(variable),
            param_to_value(value),
            Value::Null,
        ],
        Block::GetVar { variable } => vec![variable_param(variable)],
        Block::ShowVar { variable } | Block::HideVar { variable } => {
            vec![variable_param(variable), Value::Null]
        }
        Block::If { cond, body } => vec![
            param_to_value(cond),
            Value::Null,
            blocks_to_thread(body)?,
        ],
        Block::IfElse { cond, then_body, else_body } => vec![
            param_to_value(cond),
            Value::Null,
            blocks_to_thread(then_body)?,
            blocks_to_thread(else_body)?,
        ],
        Block::While { cond, body } => vec![
            param_to_value(cond),
            Value::Null,
            blocks_to_thread(body)?,
        ],
        Block::Repeat { times, body } => vec![
            param_to_value(times),
            Value::Null,
            blocks_to_thread(body)?,
        ],
        Block::Forever { body } => vec![blocks_to_thread(body)?],
        Block::Break | Block::Continue | Block::StopAll => vec![],
        Block::CalcBinOp { op, lhs, rhs } => vec![
            Value::String(op_str(*op).to_string()),
            param_to_value(lhs),
            param_to_value(rhs),
        ],
        Block::Compare { op, lhs, rhs } => vec![
            Value::String(op_str(*op).to_string()),
            param_to_value(lhs),
            param_to_value(rhs),
        ],
        Block::BoolOp { op, lhs, rhs } => vec![
            Value::String(op_str(*op).to_string()),
            param_to_value(lhs),
            param_to_value(rhs),
        ],
        Block::UnaryOp { op, expr } => vec![
        ...
        Block::Number(n) => vec![Value::from(*n)],
        Block::Text(s) => vec![Value::String(s.clone())],
        Block::Boolean(b) => vec![Value::Bool(*b)],
        Block::StringConcat { parts } => parts.iter().map(param_to_value).collect(),
        Block::StringIncludes { haystack, needle } => {
            vec![param_to_value(haystack), param_to_value(needle)]
        }
        Block::FuncCall { name, args } => vec![
        ...
        Block::FuncDef { name, params, body } => vec![
            Value::String(name.clone()),
            params.iter().map(|p| Value::String(p.clone())).collect::<Value>()
                .as_array().cloned().map(Value::Array).unwrap_or(Value::Null),
            blocks_to_thread(body)?,
        ],
        Block::Return { value } => vec![value.as_ref().map(param_to_value).unwrap_or(Value::Null)],
        Block::WhenStart | Block::WhenClick | Block::WhenCloneStart => vec![],
        Block::WhenMessageRecv { msg } => vec![Value::String(msg.clone())],
    };
